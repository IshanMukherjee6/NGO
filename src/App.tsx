...
function Home() {
    ...
    useEffect(() => {
        const timer = setInterval(() => {
            setVisible(false)
            setTimeout(() => { setCurrent(prev => (prev + 1) % slides.length); setVisible(true) }, 600)
        }, 5500)
        ...
        <AuthProvider>
            <ToastProvider>
                ...
                <Routes>
                    ...
                    <Route path="/survey" element={<ProtectedRoute><SurveyAnalysis /></ProtectedRoute>} />
                    <Route path="/survey/:folderId" element={<ProtectedRoute><SurveyFolder /></ProtectedRoute>} />
                    {/* Surveyor room — accessible after NGO surveyor sub-role selection */}
                    <Route path="/survey-room" element={<ProtectedRoute><SurveyRoom /></ProtectedRoute>} />
                </Routes>
            </ToastProvider>
        </AuthProvider>
    )
